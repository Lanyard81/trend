  ...
  return (
    <div>
      ...
      <Card style={{ marginTop: 14 }}>
        <div style={{ ...headFont, fontWeight: 800, color: PINE_T, marginBottom: 6, fontSize: 17 }}>Data & backup</div>
        <ArtifactBackup />
        <CsvExport csv={() => buildCsv(entries, water, glp, workouts, (parseFloat(profile.heightCm) || 178) / 100)} artifact />
        <ResetControls
          ...
          eraseAll={async () => { try { const r = await window.storage.list(); for (const k of (r?.keys || [])) { try { await window.storage.delete(k); } catch {} } } catch {} window.location.reload(); }}
        />
      </Card>
    </div>
  );
...
function CsvExport({ csv, artifact }) {
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <Btn kind="ghost" small onClick={go}>Export CSV (for your GP / spreadsheets)</Btn>
      {out && <textarea rows={4} readOnly value={out} style={{ ...inputStyle, fontSize: 10, marginTop: 6, resize: "vertical" }} />}
      {out && <div style={{ fontSize: 11, color: "var(--mut)", marginTop: 3 }}>Copied to clipboard — paste into Numbers/Excel, or copy from above.</div>}
    </div>
  );
}

function ArtifactBackup() {
  ...
  const doExport = async () => {
    try {
      ...
      const dump = {};
      for (const k of (r?.keys || [])) { const g = await window.storage.get(k); if (g) dump[k] = g.value; }
      const json = JSON.stringify(dump);
      setText(json); setIo("export");
      try { await navigator.clipboard.writeText(json); setMsg("Copied to clipboard — paste it somewhere safe (Notes, a file)."); }
      catch { setMsg("Copy the text below and save it somewhere safe."); }
    } catch { setMsg("Export failed."); }
  };
  const doImport = async () => {
    try {
      const dump = JSON.parse(text);
      for (const [k, v] of Object.entries(dump)) { await window.storage.set(k, v); }
  ...
  return (
    <div style={{ marginBottom: 4 }}>
      <div style={{ display: "flex", gap: 10 }}>
        <Btn kind="ghost" small style={{ flex: 1 }} onClick={doExport}>Export backup</Btn>
        <Btn kind="ghost" small style={{ flex: 1 }} onClick={() => { setIo(io === "import" ? null : "import"); setText(""); setMsg(""); }}>Import backup</Btn>
      </div>
      {io && (
        <div style={{ marginTop: 8 }}>
          <textarea rows={4} value={text} onChange={(e) => setText(e.target.value)} placeholder={io === "import" ? "Paste your backup text here…" : ""} style={{ ...inputStyle, fontSize: 11, resize: "vertical" }} />
          {io === "import" && <Btn small onClick={doImport} style={{ marginTop: 6 }}>Restore this backup</Btn>}
          {msg && <div style={{ fontSize: 12, color: "var(--mut)", marginTop: 5 }}>{msg}</div>}
        </div>
      )}
    </div>
  );
