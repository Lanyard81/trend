...
async function sSet(key, value) {
  try { localStorage.setItem("tt:" + key, JSON.stringify(value)); } catch (e) { console.error(e); alert("Storage is full — try deleting an old photo."); }
}
...
function Profile({ profile, saveProfile, goal, setGoal, entries, photos, savePhotos, supps, saveSupps, theme, setTheme, colorTheme, setColorTheme, tabsEnabled, saveTabsEnabled, holidays = [], saveHolidays, water = {}, glp = {}, workouts = [] }) {
  ...
  const addPhoto = (e) => {
    ...
    reader.onload = () => {
      ...
      img.onload = async () => {
        ...
        const id = "p" + Date.now();
        try {
          localStorage.setItem("tt:photo:" + id, dataUrl);
  ...
  const removePhoto = async (id) => {
    try { localStorage.removeItem("tt:photo:" + id); } catch {}
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <Btn kind="ghost" style={{ flex: 1 }} onClick={() => {
            ...
            for (let i = 0; i < localStorage.length; i++) {
              ...
              if (k.startsWith("tt:")) dump[k] = localStorage.getItem(k);
            }
            ...
            <input type="file" accept=".json" style={{ display: "none" }} onChange={(e) => {
              ...
              r.onload = () => {
                try {
                  ...
                  Object.entries(dump).forEach(([k, v]) => { if (k.startsWith("tt:")) localStorage.setItem(k, v); });
...
function usePhotoSrc(id) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const v = localStorage.getItem("tt:photo:" + id);
        if (live && v) setSrc(v);
      } catch {}
    })();
    return () => { live = false; };
